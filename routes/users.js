...
const {checkLoggedIn, checkCorrectUser} = require("../middleware/auth");
...
router.get("/:username/to", checkCorrectUser, async function (req, res, next) {
  try {
...
router.get("/:username/from", checkCorrectUser, async function (req, res, next) {
  try {
